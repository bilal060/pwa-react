...
const SignUpPage = () => {
  ...
  const API_URI = `${process.env.REACT_APP_API_URI}/api/v1/users/signup`;
  const postSigninData = async () => {
    try {
      ...
      localStorage.clear();
      localStorage.setItem("userdata", JSON.stringify(fetchData?.data.data));
      setTimeout(() => {
        if (signInDetails.userType === "Retailer") navigate("/retailer");
        else navigate("/age");
      }, 500);
    } catch (error) {
      console.log(error);
